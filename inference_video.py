...
def get_args():
    ...
    parser.add_argument("--video-path", "-p", type=str, default="test_2.mp4", help="Path to test image")
...
def inference(args):
    ...
    out = cv2.VideoWriter("output.mp4", cv2.VideoWriter_fourcc(*'MJPG'), fps, size)
    while cap.isOpened():
        flag, frame = cap.read()
        if not flag:
            break
        ...
        with torch.no_grad():
            ...
            frame = cv2.putText(frame, predicted_class, (100, 100), cv2.FONT_HERSHEY_SIMPLEX,
